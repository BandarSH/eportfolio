<?php
function insert_task($task,$user)
{
    // validate the given task
    $new_task = validate_input($_POST["new_task"],$_POST["User"]);
    if(empty($new_task)){
        return;
    }
    // create a prepared statement to protect against SQL injections
    $insert_statement = $GLOBALS['conn']->prepare("INSERT INTO comments (comment, user, date_added, checked) VALUES(?,?, now(),0);");
    if ($insert_statement) {
        // Bind our variable to the prepared statement as a parameter
        $insert_statement->bind_param("ss", $new_task,$user); // s indicates the data type is string.
         // s indicates the data type is string.
